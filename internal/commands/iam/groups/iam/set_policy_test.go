...
func TestNewCmdSetPolicy(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			Args:    []string{"--group=test-group", "--policy-file=test-policy.json"},
		...
		},
		{
			...
			Error: "missing required flag: --group=NAME",
		},
		{
	...
	for _, c := range cases {
		...
		t.Run(c.Name, func(t *testing.T) {
			...
			setCmd := NewCmdSetPolicy(ctx, func(o *SetPolicyOpts) error {
				gotOpts = o
				return nil
			})
			setCmd.SetIO(io)

			code := setCmd.Run(c.Args)
			if c.Error != "" {
				r.NotZero(code)
