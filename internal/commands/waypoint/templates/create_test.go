...
func TestCmdTemplateCreate(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			Args: []string{
				"-n=cli-test",
				"-s", "A templates created using the CLI.",
				...
				"-l", "cli",
				"-d", "A templates created with the CLI.",
				"-t", "cli=true",
				"--readme-markdown-templates-file", "readme_test.txt",
			},
			Expect: &TemplateOpts{
				Name:                         "cli-test",
				Summary:                      "A templates created using the CLI.",
				Description:                  "A templates created with the CLI.",
