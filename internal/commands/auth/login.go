...
func NewCmdLogin(ctx *cmd.Context) *cmd.Command {
	...
	cmd := &cmd.Command{
		...
		To authenticate non-interactively, you may authenticate as a service principal. To do so, use the {{ Bold "--client-id" }}
		and {{ Bold "--client-secret" }} flags. A service principal may be created using {{ Bold "hcp iam service-principals create" }} organization
		via the HCP Portal (https://portal.cloud.hashicorp.com).

		If authenticating a workload using Workload Identity Federation, a credential file may be used to authenticate by passing the Path
		to the credential file using {{ Bold "--cred-file" }}. The command should be running in the environment that Workload Identity was
		previously configured to be able to retrieve and federate external credentials from.
		`),
		...
		Flags: cmd.Flags{
			Local: []*cmd.Flag{
				...
				{{ Bold "hcp iam workload-identity-providers create-cred-file" }}) or service account
				credential key file.
				`),
